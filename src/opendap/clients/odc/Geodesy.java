...
import geotrans.Accuracy;
import geotrans.AlbersEqualAreaConic;
import geotrans.AzimuthalEquidistant;
import geotrans.BNG;
import geotrans.Bonne;
import geotrans.Cassini;
import geotrans.ConversionState;
import geotrans.CylindricalEqualArea;
import geotrans.Eckert4;
import geotrans.Eckert6;
import geotrans.EquidistantCylindrical;
import geotrans.GARS;
import geotrans.GEOREF;
import geotrans.Geocentric;
import geotrans.Geodetic;
import geotrans.GeotransError;
import geotrans.GeotransWarning;
import geotrans.Gnomonic;
import geotrans.IOValue;
import geotrans.JNIEngine;
import geotrans.JNIException;
import geotrans.JNIStrtoval;
import geotrans.LambertConformalConic1;
import geotrans.LambertConformalConic2;
import geotrans.LocalCartesian;
import geotrans.MGRS;
import geotrans.Mercator;
import geotrans.MillerCylindrical;
import geotrans.Mollweide;
import geotrans.NZMG;
import geotrans.Neys;
import geotrans.ObliqueMercator;
import geotrans.Orthographic;
import geotrans.PolarStereographic;
import geotrans.Polyconic;
import geotrans.Sinusoidal;
import geotrans.Stereographic;
import geotrans.TransverseCylindricalEqualArea;
import geotrans.TransverseMercator;
import geotrans.UPS;
import geotrans.USNG;
import geotrans.UTM;
import geotrans.VanDerGrinten;
...
public class Geodesy {
	...
	boolean zInitialize( StringBuffer sbError ){
		try {
			...
			String sPath_GeoTrans_dir = Utility.sConnectPaths( sBaseDirectory, "GeoTrans" );
			String sPath_GeoTrans_libs = Utility.sConnectPaths( sPath_GeoTrans_dir, "lib" );
			...
			sbError.append("error loading GeoTrans libraries: " + t);
			return false;
		}
		return true;
	}
	...
	public boolean convert( StringBuffer sbError ){
		try {
			int iProjectionType = CoordinateTypes.GEODETIC;
			
			// set direction input
			setInput();
			if( jniStrtoval.getJNIError() || jniStrtoval.getEngineError() ){
				sbError.append( "Error: \n" + JNIErrorPrefix( INPUT, iProjectionType ) );
			} else {
				// set direction output upper_mp.setProjectionDir(IOValue.OUTPUT);
				setOutput();
				if( jniStrtoval.getJNIError() || jniStrtoval.getEngineError() ){
					sbError.append( "Error: \n" + JNIErrorPrefix( OUTPUT, iProjectionType ) );
				} else {
			...
			double x = 0, y = 0, z = 0;
			setInputCoords_XYZ( ConversionState.INTERACTIVE , INPUT, CoordinateTypes.GEOCENTRIC, x, y, z );
			// setAccuracy( double ce90, double le90, double se90 );
			
			// geotrans_engine.JNIGetGeodeticCoordinates(state, direction)
	...
	public void setEngineParams_Geodetic( int iMode, int iDirection, int iHeightType ){
		try {
			Geodetic geodeticParams = new Geodetic( iHeightType );
			geotrans_engine.JNISetGeodeticParams(iMode, iDirection, geodeticParams);
		} catch( Throwable t ) {
		}
	}
	...
	public void setInputCoords_XYZ( int state, int direction, int iProjectionType, double x, double y, double z )
	{
		try {
			switch( iProjectionType ){
				case CoordinateTypes.GEOCENTRIC:
					Geocentric geocentricCoords = new Geocentric( x, y, z );
					geotrans_engine.JNISetGeocentricCoordinates( state, direction, geocentricCoords );
					break;
				case CoordinateTypes.LOCCART:
					LocalCartesian loccartCoords = new LocalCartesian( x, y, z );
					geotrans_engine.JNISetLocalCartesianCoordinates( state, direction, loccartCoords );
					break;
                default:
