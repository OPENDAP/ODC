...
abstract public class Geodata<T extends Geodata> {
	...
	static public Geodata getOutput( JNIEngine geotrans_engine, Geodata.Projection projection, StringBuffer sbError ){
		switch( projection ){
			case Geocentric:               // x: y: z
				return Geodata_Geocentric.getOutput( geotrans_engine, sbError );
			case Geodetic:                 // latitude: longitude: height (optional)
				return Geodata_Geodetic.getOutput( geotrans_engine, sbError );
			case Perspective:
			case LocalCartesian:
	...
	static int getGeotransProjectionOrdinal( Projection e ){
		switch( e ){
