...
import opendap.clients.odc.data.Model_DirectoryTree;
...
public class Panel_Retrieve_Directory extends JPanel {
	...
	boolean zInitialize( Panel_Retrieve_SelectedDatasets parent, StringBuffer sbError ){
		...
		try {
			...
			TreeCellRenderer rendererLeaflessTreeCell = new opendap.clients.odc.gui.LeaflessTreeCellRenderer();
...
//		int xIndex = Panel_Retrieve_Directory.this.mlistDirectory.getSelectedIndex();
	...
	public void vShowMessage( String sMessage ){
		if( sMessage == null ){
			mjtaMessage.setText( "" );
		} else {
	...
	public Model_Dataset getURL_directory(){
		return murlDirectory;
	}

	public Model_Dataset getURL_file(){
		return murlFile;
	}

	public boolean setURL( final Model_Dataset url, StringBuffer sbError ){
		try {
			if( url == null ){
