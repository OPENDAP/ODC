...
public class Model_Dataset_Local implements java.io.Serializable {
	...
	public static Model_Dataset_Local create( Model_Dataset model, StringBuffer sbError ){
		...
		while( variable_list.hasMoreElements() ){
			...
			Node node = zLoadNode( local.dataset_root, bt, sbError ); 
			if( node == null  ){
				sbError.insert( 0, "error loading variable " + xVariable + " " + bt.getClearName() + " of " + local.sDatasetName + ": " );
				return null;
			}
			local.dataset_root.subnodes.add( node );
		}
	...
	private static Node zLoadNode( Node parent_node, BaseType bt, StringBuffer sbError ){
		DAP_VARIABLE variable_type = DAP.getVariableType( bt, sbError );
		if( variable_type == null ) return null; // an error occurred
		Node new_node = null;
		switch( variable_type ){
			case Structure:
				new_node = new Node(); break;
			case Grid:
				new_node = new Node_Grid(); break;
			case Sequence:
				new_node = new Node_Sequence(); break;
			case Array:
				new_node = new Node_Array(); break;
			case Byte:
				new_node = new Node_Byte(); break;
			case Int16:
				new_node = new Node_Int16(); break;
			case Int32:
				new_node = new Node_Int32(); break;
			case UInt16:
				new_node = new Node_Int16(); break;
			case UInt32:
				new_node = new Node_Int32(); break;
			case Float32:
				new_node = new Node_Float32(); break;
			case Float64:
				new_node = new Node_Float64(); break;
			case String:
				new_node = new Node_String(); break;
		}
		...
		switch( variable_type ){
			case Structure:
				...
				while( member_list.hasMoreElements() ){
					...
					Node nodeMember = zLoadNode( new_node, btMember, sbError );
					if( nodeMember == null  ){
			...
			case Grid:
				...
				try {
					...
					Node_Array nodeArray = (Node_Array)zLoadNode( nodeGrid, darrayValues, sbError );
					if( nodeArray == null  ){
			...
			case Sequence:
				...
				while( field_templates.hasMoreElements() ){
					...
					if( iRowCount == 0 ){ // recursively load template
						Node nodeField = zLoadNode( nodeSequence, btFieldTemplate, sbError ); 
						if( nodeField == null ){
